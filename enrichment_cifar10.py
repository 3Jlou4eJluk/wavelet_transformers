...
class WaveletEnrichment(torch.nn.Module):
    def __init__(self, embedding_dim=384, patch_size=8, n_levels=3, wavelet='haar'):
        super(WaveletEnrichment, self).__init__()
        self.wave_transform = partial(
            ptwt.wavedec2,
            wavelet=wavelet,
            level=n_levels
        )
        self.input_size = sum([(patch_size // (2 ** i)) ** 2 * 9 for i in range(1, n_levels + 1)])
        self.n_levels = n_levels
        self.patch_size = patch_size

        self.patch_linear_proj = torch.nn.Linear(
            3 * (patch_size ** 2), embedding_dim
        )
        
        self.wavelet_linear_proj = torch.nn.Linear(
            self.input_size, embedding_dim
        )

    def forward(self, x):
        wavelet_decomp = self.wave_transform(x)
        ...
        for i in range(1, self.n_levels + 1):
            wavelet_addition = einops.rearrange(
                list(wavelet_decomp[-i]), "mat_no bs c (h ph) (w pw) -> bs (h w) (mat_no c ph pw)", 
                ph=(self.patch_size // (2 ** i)), pw=(self.patch_size // (2 ** i))
            )
            wavelet_features = (
                torch.cat((wavelet_features, wavelet_addition), dim=-1) 
                    if wavelet_features is not None 
                else wavelet_addition
            )

        final_embeddings = self.patch_linear_proj(patches) + self.wavelet_linear_proj(wavelet_features)
    ...
    def __init__(
        ...
        if mode == 'spt':
            ...
            self.linear_proj = nn.Sequential(
                ...
                nn.Linear(
                    patch_size ** 2 * im_channel, embed_size
                )
            )
            ...
            self.linear_proj = WaveletEnrichment(
                embed_size, patch_size, wavedec_levels,
                wave
            )
...
if not os.path.isfile('checkpoints/best_container'):
    model = UniversalTransformer(
        ...
        mode='enrichment', enable_lsa=True,
    ...
    base_optimizer = torch.optim.AdamW
    optimizer = SAM(
        model.model.parameters(), base_optimizer, lr=2e-5
    )
    # good parameters: cycle = 6, eta_min=5e-6. Optimizer lr = 2e-5
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, 6, eta_min=5e-6, T_mult=1
