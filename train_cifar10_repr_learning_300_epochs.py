...
class WaveDecEmbedding(torch.nn.Module):
    def __init__(self, embedding_dim=256, patch_size=8, level=3):
        ...
        self.wavedec_projection = torch.nn.LazyLinear(
            out_features=embedding_dim
        )
        self.color_projection = torch.nn.Linear(
            patch_size ** 2, embedding_dim
        )
    ...
    def forward(self, x):
        ...
        for lvl in range(1, self.level + 1):
            decomp = waves[self.level - lvl]
            decomp = einops.rearrange(
                decomp, "b (h ph) (w pw) -> b (h w) (ph pw)",
                ph = self.patch_size // (2 ** lvl),
                pw = self.patch_size // (2 ** lvl)
            )
            decomp_features = (decomp if decomp_features is None else torch.cat((decomp_features, decomp), dim=-1))
...
if not os.path.isfile('checkpoints/best_container'):
    model = UniversalTransformer(
        ...
        mode='channels_learning', enable_lsa=True,
    ...
    optimizer = SAM(
        model.parameters(), base_optimizer, lr=4e-6
    )
